<script lang="ts">
	...
	const updateLdapServerHandler = async () => {
		...
		LDAP_SERVERS = LDAP_SERVERS.map((server) => {
			if (server.enable_group_management && !server.attribute_for_groups?.trim()) {
				return { ...server, attribute_for_groups: 'memberOf' };
			}
			return server;
		});
		...
		await Promise.all([
			...
			(async () => {
				...
				LDAP_SERVERS = (ldapServersResponse?.servers || []).length
					? ldapServersResponse.servers.map((server) => ({ ...defaultLdapServer, ...server }))
					: [defaultLdapServer];
			})(),
			(async () => {
			...
			>
				...
				<div class="flex flex-col gap-3">
					{#each LDAP_SERVERS as server, index}
						<div
							class="rounded-lg border border-gray-100/50 p-3 dark:border-white/[0.04]"
						>
							<div class="mb-2 flex items-center justify-between">
								<div class="text-xs font-medium text-gray-700 dark:text-gray-300">
									{$i18n.t('Server')} {index + 1}
								</div>
							...
							<AdminSettingRow
								...
								let:labelId
							>
								<Switch bind:state={server.use_tls} ariaLabelledbyId={labelId} />
							</AdminSettingRow>
							...
							{#if server.use_tls}
								...
								<AdminSettingRow
									...
									let:labelId
								>
									<Switch bind:state={server.validate_cert} ariaLabelledbyId={labelId} />
								</AdminSettingRow>
							...
							<AdminSettingRow
								...
								let:labelId
							>
								<Switch bind:state={server.enable_group_management} ariaLabelledbyId={labelId} />
							</AdminSettingRow>

							{#if server.enable_group_management}
								<AdminSettingRow
									label={$i18n.t('Auto-Create Groups')}
									description={$i18n.t('Create missing groups from LDAP groups.')}
									let:labelId
								>
									<Switch bind:state={server.enable_group_creation} ariaLabelledbyId={labelId} />
								</AdminSettingRow>
								...
								>
									<Tooltip content={$i18n.t('Default to memberOf')} placement="top-start">
										<input
											...
											bind:value={server.attribute_for_groups}
										/>
									</Tooltip>
								</AdminSettingField>
							{/if}
					...
					<button
						type="button"
						class="rounded bg-blue-100 px-3 py-1 text-xs text-blue-700 hover:bg-blue-200 dark:bg-blue-900 dark:text-blue-300 dark:hover:bg-blue-800"
