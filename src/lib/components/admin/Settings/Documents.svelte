			...
			{#if !RAGConfig.BYPASS_EMBEDDING_AND_RETRIEVAL}
				<AdminSettingSection title={$i18n.t('Embedding')}>
					...
					{#if !RAGConfig.RAG_FULL_CONTEXT}
						<AdminSettingRow
							label={$i18n.t('File Attachments Default to Full Context')}
							description={RAGConfig.RAG_FILE_FULL_CONTEXT
								? $i18n.t(
							...
							let:labelId
						>
							<Switch bind:state={RAGConfig.RAG_FILE_FULL_CONTEXT} ariaLabelledbyId={labelId} />
						</AdminSettingRow>
					{/if}
