...
<div class="flex items-center translate-x-0.5">
	<Tooltip content={$i18n.t('Terminal')} placement="top">
		<button
			type="button"
			aria-label={$selectedTerminalId ? $i18n.t('Disable Terminal') : $i18n.t('Enable Terminal')}
			aria-pressed={!!$selectedTerminalId}
			on:click|preventDefault={toggleTerminal}
			class="flex items-center gap-1.5 translate-y-[1px] text-[13px] transition rounded-lg cursor-pointer p-1 {$selectedTerminalId
				? 'text-gray-600 dark:text-gray-300 hover:bg-gray-50/40 dark:hover:bg-gray-800/40 hover:text-gray-700 dark:hover:text-gray-200'
