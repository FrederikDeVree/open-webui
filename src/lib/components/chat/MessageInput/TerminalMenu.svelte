<script lang="ts">
	...
	import { settings, terminalServers, selectedTerminalId, user, showSettings } from '$lib/stores';
	...
	$: selectedLabel = (() => {
		...
		const direct = directTerminals.find((t) => t.url === $selectedTerminalId);
		if (direct) return direct.name || direct.url.replace(/^https?:\/\//, '');
		const system = systemTerminals.find((t) => t.id === $selectedTerminalId);
		if (system) return system.name || $i18n.t('Terminal');
		return '';
	})();
	...
	const selectDirect = (terminal: (typeof directTerminals)[number]) => {
		selectedTerminalId.set(terminal.url);
	...
	const selectSystem = (terminal: (typeof systemTerminals)[number]) => {
