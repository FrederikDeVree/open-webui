...
class ChatTable:
    ...
    async def archive_chats_by_ids_and_user_id(
        self, chat_ids: list, user_id: str, db: AsyncSession | None = None
    ) -> int:
        """Archive (or unarchive) a set of chats. Returns count of updated chats."""
        try:
            async with get_async_db_context(db) as db:
                result = await db.execute(
                    update(Chat)
                    .filter(Chat.id.in_(chat_ids), Chat.user_id == user_id)
                    .values(archived=True, updated_at=int(time.time()))
                )
                await db.commit()
    ...
    async def delete_chats_by_ids_and_user_id(
        ...
        """Delete a set of chats by id. Returns count of deleted chats.

        Cleans up shared chat snapshots, automation run references, chat messages,
        and orphan tags.
        """
        try:
            async with get_async_db_context(db) as db:
                ids_to_delete = list(set(chat_ids))

                # Collect tag ids from all chats to clean up orphans
                tag_sets: list[list[str]] = []
                for cid in ids_to_delete:
                    chat = await db.get(Chat, cid)
                    if chat:
                        tag_sets.append(chat.meta.get('tags', []))

                # Clear automation run references
                await db.execute(
                    update(AutomationRun).filter(AutomationRun.chat_id.in_(ids_to_delete)).values(chat_id=None)
                )

                # Delete shared chat snapshots for these chats
                for cid in ids_to_delete:
                    await self.delete_shared_chat_by_chat_id(cid, db=db)

                # Clear share_id on all chats (in case any were shared)
                await db.execute(
                    update(Chat).filter(Chat.id.in_(ids_to_delete), Chat.user_id == user_id).values(share_id=None)
                )

                # Delete chat messages
                await db.execute(delete(ChatMessage).filter(ChatMessage.chat_id.in_(ids_to_delete)))

                # Delete the chats
                result = await db.execute(delete(Chat).filter(Chat.id.in_(ids_to_delete), Chat.user_id == user_id))
                await db.commit()
                deleted = result.rowcount

                # Delete orphan tags (with threshold=1 since chats still exist at query time)
                for tag_ids in tag_sets:
                    if tag_ids:
                        await self.delete_orphan_tags_for_user(tag_ids, user_id, threshold=1, db=db)

                return deleted
        except Exception:
            return 0

    async def update_chat_folder_id_by_ids_and_user_id(
        self,
        chat_ids: list,
        user_id: str,
        folder_id: str | None,
        ...
        """Move a set of chats to a folder (or remove from folder if folder_id is None).
        Returns count of updated chats.
        """
        try:
            async with get_async_db_context(db) as db:
                result = await db.execute(
